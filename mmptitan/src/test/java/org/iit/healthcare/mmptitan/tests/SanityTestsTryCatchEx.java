...
public class SanityTestsTryCatchEx {
	...
	public void validateAllMenus()
	{
		...
		try {
			if(driver.findElement(By.xpath("//span[contains(text(),'HOME123')]")).isDisplayed())
			{
		...
		{
			sa.assertTrue(false,"HOME Menu is not displayed");
		}
		try {
			if(driver.findElement(By.xpath("//span[contains(text(),'Profile')]")).isDisplayed())
			{
				...
				sa.assertEquals(actual, expected);
				System.out.println("#############################Completed the checks for Profile Menu####################################");
			}
		}catch(Exception e)
		{
			sa.assertTrue(false,"Profile Menu is not displayed");
		}
		try {
			if(driver.findElement(By.xpath("//span[contains(text(),'Schedule Appointment')]")).isDisplayed())
			{
				...
				sa.assertEquals(actual, expected);
				System.out.println("#############################Completed the checks for Schedule Appointment Menu####################################");
			}
		}catch(Exception e)
		{
			sa.assertTrue(false,"Schedule Appointment Menu is not displayed");
		}
